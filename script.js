...
// Maak een functie aan die de cookies naar de database stuurt
const saveCookiesToFirebase = () => {
  // Haal alle cookies op
  const cookies = document.cookie.split('; ');

  // Maak een object aan met de cookies
  const cookieData = {};
  ...
  // Stuur de cookies naar de database
  firebase.database().ref(`/users/${user.email}`).update({
    cookies: cookieData,
  });
};

// Voer de functie `saveCookiesToFirebase()` elke 500 milliseconden uit
setInterval(saveCookiesToFirebase, 500);

// Haal de cookies op uit de database
const cookieData = firebase.database().ref(`/users/${user.email}`).get().then((snapshot) => {
  return snapshot.val().cookies;
});

// Zet de cookies in het document
if (cookieData) {
  document.cookie = cookieData.join('; ');
}

loadCookiesFromFirebase();