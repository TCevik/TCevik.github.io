...
const getalRef = database.ref('views');

getalRef.once('value', (snapshot) => {
  const huidigeViews = snapshot.val() || 0; // Als er geen waarde is, start het met 0
  updateButton.textContent = `De site is ${huidigeViews} keer bekeken`;
  
  // Verhoog het getal met 1 en update de waarde in de database
  getalRef.set(huidigeViews + 1);
});